...
# Package provides true BayesR implementation via BayesRR function (not available in BGLR 1.1.4)
library(bWGR)
...
for (i in 1:k) {
  ...
  # Fit BayesR model using bWGR package
  # BayesR: Mixture of normal distributions with different variances
  # Captures markers with small, medium, and large effects
  # Note: bWGR uses "BayesRR" function name (not "BayesR")
  # Note: bWGR creates temporary .dat files during MCMC (harmless warnings may appear)
  fm <- suppressWarnings(BayesRR(y = y_train, 
                                X = X_train, 
...
write.csv(matrics, "Results/10foldcv_bayesr.csv", row.names = FALSE)

# Print summary
print("BayesR Cross-Validation Results (using bWGR::BayesRR function):")
