...
for (i in 1:k) {
  ...
  # Fit BayesA model
  # BayesA: Each marker has its own variance (t-distribution prior)
  fm <- BGLR(y = y, 
             ETA = list(Markers = list(X = X, model = "BayesA")),
             nIter = 10000, 
             burnIn = 2000, 
             verbose = FALSE)
...
write.csv(matrics, "Results/10foldcv_bayesa.csv", row.names = FALSE)

# Print summary
print("BayesA Cross-Validation Results:")
