...
for (i in 1:k) {
  ...
  # Fit BayesC model
  # BayesC: Common variance for all markers (normal distribution prior)
  fm <- BGLR(y = y, 
             ETA = list(Markers = list(X = X, model = "BayesC")),
             nIter = 10000, 
             burnIn = 2000, 
             verbose = FALSE)
...
write.csv(matrics, "Results/10foldcv_bayesc.csv", row.names = FALSE)

# Print summary
print("BayesC Cross-Validation Results:")
